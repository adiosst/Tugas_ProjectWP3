...
class Pinjam extends MY_Controller 
{
	...
	public function index(){
		...
		if(!empty($cek) && $stts=='member')
		{
			...
			//$tmp['content']=$this->load->view('member/pinjam/View_pinjam',$data, TRUE);
			$tmp['content']=$this->load->view('member/pinjam/View_pinjam',$data, TRUE);
	...
	public function vw_kembali(){
		...
		if(!empty($cek) && $stts=='member')
		{
			...
			//$data['isi']=$this->Anggota_model->get_all();
			//$data['js']=$this->load->view('admin/anggota/js');
			$tmp['content']=$this->load->view('member/pinjam/View_kembali',$data, TRUE);
			$this->load->view('member/layout',$tmp);
		}
		else
		/*jika status login NO atau status bukan admin kembalikan ke login*/
			...
			if($_SERVER['REQUEST_METHOD'] == "POST")
			{
				...
				if( $formSubmit == 'simpan' )
				{
					...
					if($this->form_validation->run()==FALSE)
					{
						...
						$tmp['content']=$this->load->view('member/pinjam/Create_pinjam',$data, TRUE);
						$this->load->view('member/layout',$tmp);
					}
				 	else
					{
			            ...
			            redirect('member/Pinjam','refresh');
					}
				}
				else
				{
				 	...
				 	else
					{
			           ...
			           //$data['data_detail_pinjam'] = $this->Buku_model->getAllData("tb_detail_pinjam");
			            $tmp['content']=$this->load->view('member/report/report_per_anggota',$data, TRUE);
			            $this->load->view('member/layout',$tmp);
					}
	...
	public function createdet()
	{
		...
		if ($this->form_validation->run()==false) {
					...
					$data['data_anggota'] = $this->Buku_model->getAllData("tb_anggota");
					$tmp['content']=$this->load->view('member/pinjam/Create_detail_pinjam',$data, TRUE);
					$this->load->view('member/layout',$tmp);
		}
		else
		{
		    ...
		    redirect('member/Pinjam/updatedetnobuku');
		}
	}
	public function updatedetnobuku()
	{
		...
		if ($this->form_validation->run()==false) {
					...
					$tmp['content']=$this->load->view('member/pinjam/Create_detail_pinjam_nobuku',$data, TRUE);
					$this->load->view('member/layout',$tmp);
		}
		else
		{
		            ...
		            //echo $id_pinjam2;
		    		redirect('member/Pinjam/View_dt_pinjam/'.$id_pinjam2,'refresh');
		}
	}
	public function View_dt_pinjam($id_pinjam2=0)
	{
		...
		if(!empty($cek) && $stts=='member')
		{
			...
			$tmp['content']=$this->load->view('member/pinjam/View_detail_pinjam',$data, TRUE);
			$this->load->view('member/layout',$tmp);
		}
		else
		/*jika status login NO atau status bukan admin kembalikan ke login*/
	...
	{ 
		...
		{		
				...
				if($this->form_validation->run()==FALSE)
				{
					...
					$data['data_anggota'] = $this->Buku_model->getAllData("tb_anggota");
					$tmp['content']=$this->load->view('member/pinjam/Edit_pinjam',$data, TRUE);
					$this->load->view('member/layout',$tmp);
				}
				else
				{
 	...
 	public function delete_det()
		{
			...
			redirect('member/Pinjam/View_dt_pinjam/'.$id_pinjam,'refresh');
 		}
		...
		$stts = $this->session->userdata('stts');
			if(!empty($cek) && $stts=='petugas')
			{
				...
				$this->Buku_model->deleteData('tb_pinjam',$hapus);
				$this->Buku_model->deleteData('tb_detail_pinjam',$hapus);
				 redirect('member/Pinjam/vw_kembali','refresh');
			}
			else
			{
				header('location:'.base_url().'web/log');
