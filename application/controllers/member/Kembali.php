...
class Kembali extends CI_Controller {
    ...
    public function index()
    {
        ...
        if(!empty($cek) && $stts=='member')
        {
           ...
           $data['data_anggota'] = $this->Buku_model->getAllData("tb_anggota");
            ...
            $tmp['content']=$this->load->view('member/kembali/Form_kembali',$data, TRUE);
    ...
    public function View_dt_pinjam($id_pinjam2=0)
    {
        ...
        if(!empty($cek) && $stts=='member')
        {
            ...
            $tmp['content']=$this->load->view('member/Kembali/View_detail_pinjam_kembali',$data, TRUE);
            $this->load->view('member/layout',$tmp);
        }
        else
        /*jika status login NO atau status bukan admin kembalikan ke login*/
     ...
     public function detail_kembali()
    {
        ...
        if(!empty($cek) && $stts=='member')
        {
            ...
            //$data['js']=$this->load->view('admin/anggota/js');
            $tmp['content']=$this->load->view('member/kembali/Detail_kembali',$data, TRUE);
            $this->load->view('member/layout',$tmp);
        }
        else
        /*jika status login NO atau status bukan admin kembalikan ke login*/
     ...
     public function kembalikan($id=0)
    {
        ...
        if(!empty($cek) && $stts=='member')
        {
            ...
            $tmp['content']=$this->load->view('member/kembali/Buku_kembali',$data, TRUE);
            $this->load->view('member/layout',$tmp);
             //redirect('member/Kembali','refresh');
        }
        else
        /*jika status login NO atau status bukan admin kembalikan ke login*/
        {
        ...
        public function vw_dt_back()
    {
        ...
        if(!empty($cek) && $stts=='member')
        {
           ...
           $data['data_pinjam'] = $this->Buku_model->get_detail1("tb_pinjam","id_pinjam",$id_pinjam);
           $data['data_detail_pinjam'] = $this->Buku_model->getAllData("tb_detail_pinjam");
            $tmp['content']=$this->load->view('member/kembali/Buku_kembali_strc',$data, TRUE);
            $this->load->view('member/layout',$tmp);
        }
        else
        /*jika status login NO atau status bukan admin kembalikan ke login*/
